...
order by ratio;
----------------------------
--waq to find data where region is central and quantity >6
select * 
from orders_data
 where region='Central' and quantity>6;
 ----------------------
 --waq to find data where region is central or category is technoligy and quantity>6
 Select * 
 from orders_data
 where (region ='Central'or category='Technology') and quantity>6;

 -------------------------------
 ---between  WAQ to find data where quantity between 3 and 5
 select* 
 from orders_data
 where quantity between 3 and 5;
 -----------
 In
 --WAQ to fetch data where city is los angeles or sanfrancisco
 select *
 from orders_data
 ...
 --Q> customer name starting with A
 select *
 from orders_data
 where customer_name like'A%';
 -------------------------
 --WAQ to fetch data where 2nd character in name is either a or e and the name ends with n
 select *
 from orders_data
 ...
 from orders_data;
 select sum(sales)/count(*) as avg_sale
 from orders_data
 ---------
 update orders_data
 set city=null 
 ...
 select count(*),count(city),count(order_id)
 from orders_data;

 select distinct(region)
 from orders_data;

 select count(distinct city) as dist_city, count(distinct category)
 from orders_data;
 --------------------------
 --groupby
 --Q> WAQ to fetch details of sales done in each region category wise
 
 select category,region,sum(sales) as categ_sales
 from orders_data
 group by category,region
 order by region;
 
 -- Fetch cities belonging to west region and having sales>500
 select * from orders_data;
 select * from returns_data;

 select city,count(city),cast(sum(sales) as int)
 from orders_data
 where region='West'
 group by city
...
-- join
Q> WAQ to find total sales of all the return orders due to any reason

select r.return_reason ,sum(o.sales) as total_return_sale
from orders_data o
join returns_data r on o.order_id = r.order_id
group by return_reason
------------------------------
Q> WAQ to find total sales of all the returned orders based on order_id

select o.order_id,count(1),sum(sales) as total_return_sales
from orders_data o
 join returns_data r on o.order_id=r.order_id
group by o.order_id
 -----------------------------------------------------
 Q> WAQ to fetch data of items not returned along with the total sales and profit/loss.

 select o.order_id,count(1) as c ,sum(sales) as total_sales ,sum(profit) as total_profit
 from orders_data o
 left join returns_data r on o.order_id=r.order_id
 where r.return_reason is null
 group by o.order_id
 -----------------------------
 Q> display total sales of all returned reason as well as non returned products

 select r.return_reason,case when return_reason='Wrong Item'then 'wrong Items' else return_reason end as new_return_reason,
 ...
 ---------------
 select *, case when return_reason='Wrong Item'then 'wrong Items' else return_reason end as new_return_reason, distinct new_return_reason
 from returns_data;
